                ...
                if (event.getAction() == MotionEvent.ACTION_UP)
                {
                    ...
                    if (Math.abs(mUpX - mTouchX) <= mTouchSlop && Math.abs(mUpY - mTouchY) <= mTouchSlop)
                    {
                        for (int i = 0; i < mValueTotalList.size(); i++)
                        {
                            ...
                            {
                                setSelected(i);
                                Log.v("3699点击", "" + i+"--------------"+(mScroller.getCurrX() + mUpX) + "---" + mScroller.getCurrX() + "---" + mUpX);
//                                mScroller.setFinalX((mScroller.getCurrX()));
//                                computeScroll();
//                                mScroller.startScroll(mScroller.getFinalX(), mScroller.getCurrY(),
//                                        (mMeasureWidth / 5), 0, 400);

                                if (Math.abs((mUpX - mMeasureWidth / 2)) <= 15)
                                    {

                                } else {

                                    int space = mMeasureWidth / 5;
                                    if (mUpX < (space / 2 + space)) {
                                        Log.e("3699点击", "aaaa");
                                        mScroller.startScroll(mScroller.getFinalX(), mScroller.getCurrY(),
                                                -(mMeasureWidth / 5) * 2, 0, 400);
                                    } else if (mUpX >= (space / 2 + space) && mUpX < (space / 2 + 2 * space)) {
                                        Log.e("3699点击", "bbbbb");
                                        mScroller.startScroll(mScroller.getFinalX(), mScroller.getCurrY(),
                                                -(mMeasureWidth / 5), 0, 400);
                                    } else if (mUpX > (mMeasureWidth / 2 + space)) {
                                        Log.e("3699点击", "cccc");
                                        mScroller.startScroll(mScroller.getFinalX(), mScroller.getCurrY(),
                                                (mMeasureWidth / 5), 0, 400);
                                    } else {
...
//3699点击
            Log.v("3699停止", mScroller.getCurrX() + "    " + mScroller.getFinalX() + "      " + mMeasureWidth / 5 + "   lv:" + lv + "---" + n);
            Log.v("3699停止", (mMeasureWidth / 10) + "--n");
